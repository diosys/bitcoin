...
/** Amount of diosyss spent by the transaction.
...
static const int64 nTargetSpacing = 45;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DiosysMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DiosysMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DiosysMiner(CWallet *pwallet)
{
    printf("DiosysMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("diosys-miner");
    ...
    try { loop {
        ...
        printf("Running DiosysMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DiosysMiner terminated\n");
        throw;
    }
}

void GenerateDiosyss(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DiosysMiner, pwallet));
}
